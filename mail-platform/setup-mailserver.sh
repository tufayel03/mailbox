...
configure_panel_env() {
  ...
  if [[ -f "$env_file" ]]; then
    ...
    set_env_value "$env_file" "MAILBOX_STATS_CMD" "sudo -n /bin/bash $SCRIPT_DIR/panel/mailbox-stats.sh {{EMAIL}}"
    set_env_value "$env_file" "MAILBOX_PURGE_CMD" "sudo -n /bin/bash $SCRIPT_DIR/panel/mailbox-purge.sh {{EMAIL}}"
